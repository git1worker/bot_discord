...
async def timer_messages(num_messages_in_5_sec):
    global flag
    global messages
    ...
    for k, v in messages.items():
        ...
        if v > num_messages_in_5_sec:  # Количество сообщений за 5 секунд # Антиспам!
            await antispam(k)
        cursor.execute(f"""UPDATE dis_users SET time_afk = 0 WHERE id_discord = {k}""")
        cursor.execute(f"""SELECT * FROM dis_users WHERE id_discord = {k}""")
...
async def timer_min():
    ...
    while True:
        ...
        for member in members:
            if member.bot:
                continue
            if member.voice is not None and member.voice.channel != afk_voice:

                cursor.execute(f"""SELECT * FROM dis_users WHERE id_discord = {member.id}""")
                num = cursor.fetchone()[3]
                cursor.execute(f"""UPDATE dis_users SET time_on_voice = {num + 1} WHERE id_discord = {member.id}""")  # Счет времени в войсе
                cursor.execute(f"""UPDATE dis_users SET time_afk = 0 WHERE id_discord = {member.id}""")  # Обнуление счетчика афк
                conn.commit()
            cursor.execute(f"""SELECT * FROM dis_users WHERE id_discord = {member.id}""")
...
async def on_ready():
    ...
    text_chann = client.get_channel(938411811173199955)  # текстовый канал с которым взаимодействует bot
    guild = client.get_guild(938411811173199952)  # айди сервера
    new_role = guild.get_role(941606299144171591)  # роль новичка
    afk_voice = guild.get_channel(944144703148937216)  # Объект афк канала
    dead = guild.get_role(944202377500717126)  # Труп
    conn = sqlite3.connect("./mydatabase.db")
    cursor = conn.cursor()
    cursor.execute(f"""UPDATE dis_users SET warnings = 0 WHERE id_discord = 869217405862305822""")
    conn.commit()
    await timer_min()
    ...
    if message.channel.id == 938411811173199955:  # Проверка на основной канал
        ...
        if not flag:
            if len(str(message.content)) > 400:  # Если больше 400 сим удаление и предупреждение
                ...
                await timer_messages(3)  # Если меньше 200 символов за 3 секунд ограничение - 4 сообщений
...
@client.command()
async def get_info_voice(ctx, arg):
    global cursor
    try:
...
@client.command()
async def get_info_messages(ctx, arg):
    global cursor
    try:
        cursor.execute(f"""SELECT * FROM dis_users WHERE id_discord = {arg}""")
        user = guild.get_member(int(arg))
...
@client.command()
async def get_info_warnings(ctx, arg):
    global cursor
    try:
        cursor.execute(f"""SELECT * FROM dis_users WHERE id_discord = {arg}""")
        user = guild.get_member(int(arg))
